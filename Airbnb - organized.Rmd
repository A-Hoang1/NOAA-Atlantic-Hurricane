...
```{r}
# Normalizing and standardizing the explanatory variables
clean3_norm <- 
  listings_clean3 |> 
  mutate(
    across(
...
price_df
```
Taking a look at our fit statistics, we can see that our best choice of k is 48 with an R2 of 0.4489 and MAE of 0.1609. Our model's Mean Absolute Error (MAE) of 0.1609 is lower than the baseline MAE_mean of 0.2208 (the error we'd get by always predicting the mean), leading to a Mean Absolute Relative Error (MARE) of 0.2713. This indicates that while our model performs better than simply predicting the mean price, it still has significant room for improvement. The RMSE of 0.203 is relatively close to our MAE, suggesting our errors are fairly consistent without extreme outliers. These statistics collectively suggest that kNN regression isn't great for predicting prices with just bedrooms and bathrooms as predictors. Leaving out room_type, which is a binary variable but also one that contributes significantly in predicting prices, affects the overall accuracy of our model. Given that our model only explains about 44.89% of the variance in prices, a different modeling approach that can incorporate both numeric and categorical variables (such as a regression tree) might be more appropriate.

```{r}
listings_clean3 |> 
  pivot_longer(
    cols = c(bedrooms, bathrooms, room_type),  # including room_type
    names_to = "variable",
    values_to = "value"
  ) |> 
  mutate(
    variable = as_factor(variable),
    high_price = if_else(log10_price > median(log10_price), "red", "black")
  ) |> 
...
```
Looking at the relationships between our predictors and price, we can see clear non-linear patterns, particularly in how bathrooms and bedrooms relate to price, as shown by the curving blue LOESS lines. Additionally, room_type shows distinct price groupings, suggesting clear categorical effects.

```{r}
RNGversion("4.1.0")
set.seed(2870)

# Create regression tree with bedrooms, bathrooms, and room_type
airbnb_full <- 
 rpart(
...
data.frame(airbnb_full$cptable)
```
...
caret::varImp(airbnb_tree) |> 
 ...
 labs(x = NULL,
      ...
      title = "Importance of Variables in Predicting Airbnb Prices") +  # added title
 
 scale_y_continuous(expand = c(0, 0, 0.05, 0)) +
 
 theme_minimal() +  # optional: for cleaner look
 theme(
   axis.text.x = element_text(angle = 45, hjust = 1)  # for better label readability
 )
```
...
# compare the distribution of predicted vs. actual log10_prices
bind_rows(
...
# how many cases are in each of our nodes
pred_results |>
 count(node) |>
 arrange(desc(n))

# Now let's make the graph of predicted vs actual values
ggplot(
 data = pred_results,
 mapping = aes(
   x = predicted, 
 ...
 geom_point(
   ...
   show.legend = F,
   alpha = 0.5  # added for better visibility with large dataset
 ) +
 labs(
   x = "Predicted log10(Price)",
   y = "Actual log10(Price)",
   title = "Predicted vs Actual Airbnb Prices by Tree Node"
 ) +
 theme_minimal()
```