...
```
The residual heatmap of the regression tree predicts Airbnb prices based on our predictor variables. Node 26 (listings with more than 2 bedrooms, fewer than 1.75 bathrooms, and classified as private/shared rooms) demonstrates significant overprediction, likely inflated by luxury listings. While the 95% confidence intervals for this node are relatively narrow, indicating consistent pricing within this category, the consistent overestimation highlights the model's struggle with high-priced variability. In contrast, node 59 (listings with more than 3.25 bathrooms, 2.25 bedrooms, and classified as private/shared rooms) shows underprediction, where actual prices exceed predictions. The wider confidence intervals for this node suggest a greater diversity in listing prices, reflecting the model's difficulty in capturing the premium pricing accurately. Nodes like 25 (more than 2 bedrooms and fewer than 1.75 bathrooms) align more closely with actual prices and feature narrower intervals, suggesting more reliable and consistent predictions for mid-range listings.
