...
```{r}
# Manually assign the correct node labels from the tree diagram
pred_results <- listings_clean3 |>
  mutate(
    predicted = pred_price,
    node = as.numeric(row.names(airbnb_tree$frame)[airbnb_tree$where]) # Match tree nodes
  )


# Map the new node labels to match the tree diagram
node_mapping <- c(
  "8" = "1.829", "9" = "1.933", "10" = "2.058", 
  "12" = "2.142", "22" = "2.087", "25" = "2.243",
  "26" = "2.345", "27" = "2.359", "31" = "2.158",
  "58" = "2.201", "59" = "2.439", "61" = "2.516", 
  "62" = "2.616"
)



# Apply the mapping to the node column
pred_results$node <- factor(pred_results$node, levels = names(node_mapping), labels = node_mapping)

# Recalculate statistics for heatmap
ci_results <- pred_results |>
  summarise(
    mean_predicted = mean(predicted, .by = node),
    mean_actual = mean(log10_price, .by = node),
    n = sum(!is.na(log10_price)),  # Count non-NA observations
    sd_actual = sqrt(sum((log10_price - mean(log10_price, .by = node))^2) / (n - 1)),
    se_actual = sd_actual / sqrt(n),
    lower_ci = mean_actual - 1.96 * se_actual,
    upper_ci = mean_actual + 1.96 * se_actual,
    .by = node
  )

# Heatmap with improved legend for residuals
ggplot(data = ci_results, 
       aes(x = node, 
           y = mean_actual)) +
  geom_tile(
    aes(fill = mean_actual - mean_predicted),  # Residuals as fill
    color = "white",  # Add gridlines
    width = 1,        # Adjust width of tiles
    height = 0.4      # Adjust height of tiles for better visibility
  ) +
  geom_errorbar(
    aes(
      ymin = lower_ci,
  ...
  scale_fill_gradient2(
    low = "blue",                   # Underpredictions
    mid = "green",                  # Accurate predictions
    high = "red",                   # Overpredictions
    midpoint = 0,                   # Zero residuals at the center
    name = "Accuracy",             # Title for legend
    breaks = c(-0.5, 0, 0.5),       # Define corresponding breaks
  ) +
  labs(
    title = "Heatmap of Residuals by Node",
    subtitle = "Confidence intervals shown as error bars",
    x = "Node",
    y = "Mean"
  ) +
  theme_minimal() +
...
```
The heatmap of residuals reveals inaccuracies across several nodes in the regression tree. Node 2.345, representing listings with more than 2 bedrooms, fewer than 1.75 bathrooms, and classified as a private or shared room, shows significant overprediction, suggesting that outliers may still be inflating the predicted prices. In contrast, nodes such as 2.439, associated with listings featuring more than 3.25 bathrooms, exhibit underprediction, where actual prices are underestimated. Nodes like 2.243 and 2.201, which correspond to listings with moderate bathroom counts and room types, demonstrate closer alignment between predicted and actual prices. Overall, the model performs relatively well, though it generally tends to underpredict prices.
