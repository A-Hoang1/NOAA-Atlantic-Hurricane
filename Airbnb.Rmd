...
price_knn <- 
  knn.reg(
    train = listings_clean3 |> select(-price),  # Training predictors
    test = listings_clean3 |> select(-price),    # Testing predictors
    y = listings_clean3$price,                  # Training response variable
...
```
The KNN model with k=5k=5 provides close predictions for most listings, though it maybe a little overfitted as we are using the same data for testing and training. We can see by small residuals like -0.2 and 0.0 for actual prices of 399 and 49 predicted respectively well. However, larger residuals, such as -27.4 for an actual price of 110, indicate that the model struggles with certain listings, likely due to outliers. Overall, the model captures general pricing trends but could benefit from further tunings of k. 
...
price_df <- data.frame(
  model = c("price_lm7", "price_lm6","price_lm5", "price_lm4", "price_lm3", "price_lm2", "price_lm1"),
  
  # R-squared values
  r_squared = c(
    cor(listings_clean3$price, price_pred$price7)^2,
    cor(listings_clean3$price, price_pred$price6)^2,
    cor(listings_clean3$price, price_pred$price5)^2,
    cor(listings_clean3$price, price_pred$price4)^2,
    cor(listings_clean3$price, price_pred$price3)^2,
    cor(listings_clean3$price, price_pred$price2)^2,
    cor(listings_clean3$price, price_pred$price1)^2
  ),
  
  # Sigma values
  sigma = c(
    calc_sigma(listings_clean3$price, price_pred$price7),
    calc_sigma(listings_clean3$price, price_pred$price6),
    calc_sigma(listings_clean3$price, price_pred$price5),
    calc_sigma(listings_clean3$price, price_pred$price4),
    calc_sigma(listings_clean3$price, price_pred$price3),
    calc_sigma(listings_clean3$price, price_pred$price2),
    calc_sigma(listings_clean3$price, price_pred$price1)
  ),
  
  # MAE values
  mae = c(
    calc_mae(listings_clean3$price, price_pred$price7),
    calc_mae(listings_clean3$price, price_pred$price6),
    calc_mae(listings_clean3$price, price_pred$price5),
    calc_mae(listings_clean3$price, price_pred$price4),
    calc_mae(listings_clean3$price, price_pred$price3),
    calc_mae(listings_clean3$price, price_pred$price2),
    calc_mae(listings_clean3$price, price_pred$price1)
  )
)
...
price_df
```
Taking a look at predicted prices and price_lm6 with Mean Absolute Error (MAE) this indicates the model's average prediction error is smaller compared to other models and avoids overfitting. However, despite its lower MAE, the average error still suggests that the predictions deviate from the actual Airbnb prices by a notable amount.
...
```
The residual plots show consistent model performance for bedrooms and room_type, with residuals centered around $0 and no significant trends or systematic errors. However, for bathrooms, residuals show a clear downward trend as the number of bathrooms increases, indicating the model underestimates prices for listings with many bathrooms.