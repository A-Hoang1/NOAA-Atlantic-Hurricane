...
Overall, price_lm6 appears to be the best trade-off between simplicity and performance. It has the second-highest R-squared (0.440), the lowest sigma (107), and uses only three predictors (bedrooms, room_type, bathrooms), making it simpler and less prone to overfitting compared to price_lm7. The sigma still seems to be high at 106. This number means that typically, the model’s predictions of Airbnb prices are off by approximately \$106 from the actual prices which is quite high. The average prediction error looking at MAE is 70.92, which means the average prediction error is off by \$70.92.
...
fit_stats_combined |>  
  ...
  # Creating one column to store the fit statistics in 
...
From our results of our fit statistics we can see that the our R2 has improved by approximately 0.0826, which is a little better than our kNN regression model (R2 = 0.4352), while our MAE of 0.1463 improved by 0.0144, this still indicates our predictions deviate by about 0.15 on the log10 scale from actual prices, showing a mean reduction in error of 32.82% compared to baseline predictions, with these improvements in mind we should use a regression tree model rather than kNN regression. 
...
ggplot(
 ...
 labs(
   x = "Predicted",
   y = "Actual",
   title = "Pruned Regression Tree of Predicted vs Actual Airbnb Prices"
 ) +
